...
class transactionController {
    ...
    static async getTransactionForCompany(id, limit, skip) {
        try {
            return await Transaction.find({"companyId": id}).limit(limit).skip(skip).sort({"date": -1}).exec();
        }
        catch (e) {
            throw(e)
        }
    ...
    static async countTransactionForCompany(id) {
        try {
            return await Transaction.find({"companyId": id}).count().exec();
        }
        catch (e) {
            throw(e)
        }
    }
    static async upsertTransaction(id,transaction) {
        try {
