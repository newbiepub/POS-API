        ...
        if (!productCount) {
            ...
            for (let i = 0; i < 20; i++) {
                let product = {
                    ...
                    price: [{_id: faker.random.uuid(), name: faker.random.word(), price: faker.commerce.price()}],
                    unit: "cái",
                    companyId: company._id.toString(),
            ...
            let companyInventory = await new Inventory({
                productItems: _.map(productItems, (item) => {
                    return {productId: item._id, quantity: faker.random.number()}
                }),
                companyId: company._id.toString(),
            }).save();
        }
