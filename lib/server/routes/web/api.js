...
webApi.post("/company/inventory/import/csv", async (req, res, next) => {
    try {
        ...
        productData = await addPriceToArray(productData, "import", 'priceImport'); // Add import price to array
        productData = await  addPriceToArray(productData, 'default', 'priceSale'); // Add sale price to array
