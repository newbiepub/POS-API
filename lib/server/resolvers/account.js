...
export const employeeLogin = async (username, password) => {
    try {
        ...
        if (currentUser) {
            ...
            if(currentUser.status === 'deactivate') throw new Error('DEACTIVATED_USER');
...
export const exchangeToken = async (token, refresh_token) => {
    try {
        let { userId, access_token } = await AccessToken.findOne({refresh_token}).populate([{
            path: "userId",
            model: "users"
        }]).exec();

        if(userId && access_token === token) {
            return await createAuthToken(userId._id);
        }
    } catch (e) {
        console.error("ERROR - EXCHANGE TOKEN - ", e.message);
    }
