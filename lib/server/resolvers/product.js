...
export async function addNewProduct (name, price = [defaultPrice], unit, description, categoryName, companyId, productCode, quantity = 0, currency) {
    ...
    try {
       ...
       await ProductInventory.findOneAndUpdate({product: product._id}, {$set: {quantity}}, {upsert: true, new: true}).exec();
       return product;
    }
    catch(e) {
