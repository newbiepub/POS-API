...
import {createTransaction, issueRefundTransaction} from './transaction';
    ...
    Query: {
        ...
        async getTransactionEmployee(obj, args, context, info) {
            try {
                ...
                if (_.includes(context.user.roles, "employee")) {
                    let limit = args.limit || 100, skip = args.skip || 0;
                    return Transaction.find({employeeId: employeeId}).sort({"date": -1}).limit(limit).skip(skip).exec();
                }
    ...
    Mutation: {
        ...
        async createTransaction(obj, args, context, info) {
            ...
            try {
                ...
                if (_.includes(context.user.roles, "employee") && args.productItems.length > 0) {
                    let subtractInventory = await subtractProductAfterTransactionEmployee(args.productItems, employeeId);
                    return createTransaction(employeeId, companyId, args.productItems, args.type, args.paymentStatus, args.paymentMethod, args.dueDate, args.totalQuantity, args.totalPrice, args.paid, args.description, args.customer,args.date)
                }
                throw new Error("PERMISSION_DENIED");
            } catch (e) {
                throw e;
