...
import DiscountSchema from '../schemas/discount.graphql'
...
import {createTransaction, issueRefundTransaction, updateTransaction, sendMailInvoice} from './transaction';
import {GraphQLScalarType} from 'graphql';
import {Kind} from 'graphql/language';
import {InventoryMutation} from "../features/inventory";
import {InventoryQuery} from "../features/inventory/index";
import {DISCOUNT_MUTATION, DISCOUNT_QUERY} from "../features/discount/resolver";
import {PRODUCT_MUTATION} from "../features/inventory/product/resolver";
import {INVENTORY_ACTIVITY_MUTATION, INVENTORY_ACTIVITY_QUERY} from "../features/inventory/activity/resolver";
import {CATEGORY_MUTATION, CATEGORY_QUERY} from "../features/inventory/category/resolver";
import {POS_MANAGEMENT_MUTATION} from "../features/pos/resolver";
...
const resolvers = {
    ...
    Query: {
        ...
        async getTransactionEmployee(obj, args, context, info) {
            try {
                let id = context.user._id, limit = args.limit, skip = args.skip, result;
                if (context.user.roles.includes('employee')) {

                    result = await Transaction.find({employeeId: id}).sort({"createdAt": -1}).limit(limit).skip(skip).exec();
                } else {
                    result = await Transaction.find({companyId: id}).sort({"createdAt": -1}).limit(limit).skip(skip).exec();
                }
                return result;
        ...
        async getTransactionById(obj, args, context, info) {
            try {
                if(!context.user.roles.includes('company')) throw new Error("COMPANY ONLY");

                let id = args.employeeId, limit = args.limit, skip = args.skip, result;;
                result = await Transaction.find({employeeId: id}).sort({"createdAt": -1}).limit(limit).skip(skip).exec();
                return result;
            }
            catch(e) {
        ...
        getAmountTransactionEmployee: async (obj, variables, context) => {
            try {
                let id = context.user._id, result = 0;
                if (context.user.roles.includes('employee')) {
                    result = await Transaction.find({employeeId: id}).count().exec();

                } else {
                    result = await Transaction.find({companyId: id}).count().exec();
                }
    ...
    Mutation: {
        ...
        async createTransaction(obj, args, context, info) {
            try {
                ...
                if (context.user.roles.includes('employee') && args.productItems.length > 0) {
                    ...
                    return createTransaction(employeeId, companyId, args.productItems, args.paymentStatus, args.paymentMethod, args.dueDate, args.totalQuantity, args.totalPrice, args.paid, args.description, args.customer, args.createdAt, args.issueRefund, args.issueRefundReason, args.refundDate)
         ...
         * @returns {Promise.<void>}
         */
        async sendMailInvoice(obj, args, context, info) {
            try {
                ...
                return sendMailInvoice(customerEmail, transaction, profile);
                throw new Error("PERMISSION_DENIED");
            } catch (e) {
                throw e;
            }
        ...
        ...DISCOUNT_MUTATION,
        ...PRODUCT_MUTATION,
        ...INVENTORY_ACTIVITY_MUTATION,
        ...CATEGORY_MUTATION,
        ...POS_MANAGEMENT_MUTATION
    },
    CurrentCompany: {
        roles: async (user) => {
